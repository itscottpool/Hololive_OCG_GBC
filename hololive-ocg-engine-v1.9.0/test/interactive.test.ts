...
test("interactive Sora Oshi skills expose Replacement and opponent-swap choices", () => {
  ...
  const replacement = engine.state.log.findLast(entry => entry.event === "CHEER_REATTACHED");
  assert.equal(replacement?.data?.cardId, cheer.cardId);
  assert.equal(replacement?.data?.sourceStageId, center.stageId);
  assert.equal(replacement?.data?.targetStageId, back.stageId);
  assert.equal(replacement?.data?.sourceCardId, "hBP01-021");
  assert.equal(replacement?.data?.targetCardId, "hSD01-004");
  assert.equal(replacement?.data?.sourceName, "Tokino Sora");
  assert.equal(replacement?.data?.targetName, "Tokino Sora");

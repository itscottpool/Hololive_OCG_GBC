...
This project is a dependency-free TypeScript implementation of the Hololive Official Card Game rules needed to play the supplied hSD01 Tokino Sora/AZKi starter deck. Version 1.9.0 moves target selection onto the live table, anchors Art impacts to their actual targets, expands transfer narration, adds the Cheer Deck counter, and introduces a purpose-built title-screen pixel font.
...
Version 1.9.0 intentionally retains the dependency-free BAT launcher while the vertical slice is still changing quickly. Version 2.0 is the planned desktop-application packaging pass. For public distribution, the eventual executable and installer should be code-signed; an unsigned hobby executable can still trigger Windows SmartScreen and may look less trustworthy than the current transparent BAT launcher.
...
Every performed action now plays before control returns to the player. Each individual event carries its own projected board state, so the turn/phase header, middle prompt bar, field, hand totals, and pending Cheer display change only when that exact narrated event is reached. Text windows narrate CPU and player decisions in chronological order, with card reveals, Cheer displays, draw-to-hand movement, deck shuffling, phase announcements, and attacks drawn over the actual target card on the live battlefield. Cheer transfers name both Holomem and visibly move the selected Cheer from a source-card portrait to a destination-card portrait. After each animation and message finish, press Enter or click to advance to the next action. Nothing advances automatically.

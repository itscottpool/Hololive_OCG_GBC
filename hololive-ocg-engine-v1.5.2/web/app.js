...
function stageToken(holomem, position, owner) {
  ...
  token.addEventListener("click", () => openBattleCardActions(owner, holomem.stageId));
...
function oshiToken(player, owner) {
  ...
  token.addEventListener("click", () => openBattleCardActions(owner, "OSHI"));
...
function renderBackstagePeeks(player, container, owner) {
  ...
  container.replaceChildren(...player.stage.back.filter(Boolean).map(holomem => {
    ...
    peek.addEventListener("click", () => openBattleCardActions(owner, holomem.stageId));
    return peek;
  }));
}
...
function renderBattleFocus() {
  ...
  ui.eventMessage.textContent = `${item.owner === "P1" ? "YOUR" : "OPPONENT"} ${area} · ${definition?.name ?? "CARD"} · ← → SELECT · ENTER ACTIONS`;
}
...
function activateVisibleFocus() {
  ...
  if (!item) return;
  openBattleCardActions(item.owner, item.key);
}

...
This project is a dependency-free TypeScript implementation of the Hololive Official Card Game rules needed to play the supplied hSD01 Tokino Sora/AZKi starter deck. Version 1.5.0 keeps every narrated action in lockstep with its exact board state, adds detailed Art-selection rows and hand sorting, and upgrades the title screen with the official logo and its own supplied chiptune track.
...
The opening screen places the supplied official Hololive OCG logo and a console-style menu directly over the pixel-treated title artwork. `Endless Nights` uses a more decorative high-contrast subtitle treatment, while the unobtrusive build number sits in the upper-right corner. Story Mode is visibly reserved but disabled; Vs. AI opens a deck-selection screen before Oshi selection, Options contains Color Scheme and separate BGM/SFX volume controls, and Exit Game stops the local process. The supplied `Non-Fiction` 8-bit track plays across the title and setup screens, then hands off to the existing battle music when the match begins. The three interface palettes—Classic White, Retro Mint, and Moonlight—and both audio levels are remembered for later sessions on the same browser.
...
Hand and Backstage navigation remains intentionally layered: choose the zone, choose a card, choose one currently legal action, and then choose its target when applicable. The Hand browser includes a mouse-accessible `Sort Hand` control and an `S` keyboard shortcut. Sorting groups Holomem first by name and then Debut/Spot → 1st → 2nd → 3rd, followed by Support cards grouped alphabetically by name. Visible Center, Collab, and Oshi cards skip the zone list because they can be highlighted directly from the battle scene. Every card has an Inspect action that opens its artwork at nearly the full height of the 480×270 screen.

During Performance, each selectable Art row displays its Cheer cost icons, Art name, base damage, and printed effect directly in the selection list. The full card detail remains on the right for reference, but choosing an Art no longer requires moving attention away from the action list.
...
Normal PC now pauses after being played and displays every valid Debut holomem found in the player's deck. The player chooses which one is revealed and placed on the Back Stage; the remaining candidates return to the deck before it is shuffled.
...
Pressing Escape on the main battle overview opens a safe, default-to-No forfeit prompt. Confirming returns to the title without retaining the old match. The title screen's Exit command shuts down the local game server process.

The browser client runs only on `127.0.0.1` and does not expose the AI's hidden hand, deck order, Life cards, or Holo Power contents. Sub PC is fully interactive; some other nested card-effect selections still use the deterministic policy and can migrate to the pending-decision framework in later passes.

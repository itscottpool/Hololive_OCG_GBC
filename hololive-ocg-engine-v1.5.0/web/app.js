...
const ids = [
  ...
  "dialog-zone-toggle", "dialog-sort", "card-detail", "dialog-help", "action-playback", "playback-visual", "playback-speaker", "playback-text", "playback-help",
...
function openZone(command, selectedStageId = null, fieldOwner = "P1") {
  ...
  if (command === "HAND") {
    ...
    if (handSortEnabled) items = sortHandItems(items);
  }
...
function openCardActions(item) {
  ...
  const items = [
    ...
    ...[...groups.values()].map(group => {
      ...
      return { kind: "ACTION", name: actionName(action, item), tag: art ? "ART" : "ACTION", group, source: item, art };
    }),
...
function renderDialog() {
  ...
  ui.dialogSort.textContent = handSortEnabled ? "SORTED ✓" : "SORT HAND";
  ...
  ui.dialogList.replaceChildren(...view.items.map((item, index) => {
    ...
    row.className = `list-entry${index === view.index ? " selected" : ""}${item.kind === "ATTACHMENT" ? " attachment-entry" : ""}${item.art ? " art-action-entry" : ""}${firstAttachment ? " attachment-first" : ""}`;
    ...
    const mainContent = item.art
      ? `<span class="art-action-summary"><span class="art-action-heading">${requirementIcons(item.art.cost)}<strong>${escapeHtml(item.art.name)} · ${item.art.damage}</strong></span><span class="art-action-effect">${escapeHtml(item.art.printedText || "No additional effect.")}</span></span>`

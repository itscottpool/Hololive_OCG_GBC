...
const ids = [
  ...
  "inspect-dialog", "inspect-image", "inspect-name", "title-music", "battle-music", "attack-sfx", "music-toggle",
...
function playTone(frequency, duration = 0.1, type = "square", volume = 0.055, delay = 0, endFrequency = frequency) {
  ...
  gain.gain.setValueAtTime(Math.min(.42, volume * SFX_SYNTH_BOOST * sfxLevel / 10), start);
...
function playNoise(duration = 0.12, volume = 0.035, delay = 0, filterFrequency = 1800) {
  ...
  gain.gain.setValueAtTime(Math.min(.36, volume * SFX_SYNTH_BOOST * sfxLevel / 10), start);
  gain.gain.exponentialRampToValueAtTime(0.0001, start + duration);
...
function playAttackSfx() {
  if (sfxLevel === 0) return;
  const music = activeMusicTrack();
  const originalMusicVolume = music?.volume ?? 0;
  if (music && !music.paused) music.volume = originalMusicVolume * .42;
  ui.attackSfx.pause();
  ui.attackSfx.currentTime = 0;
  ui.attackSfx.volume = Math.min(1, .95 * sfxLevel / 10);
  const restoreMusic = () => {
    if (music) music.volume = bgmLevel / 10;
  };
  ui.attackSfx.play().catch(restoreMusic);
  ui.attackSfx.addEventListener("ended", restoreMusic, { once: true });
  window.setTimeout(restoreMusic, 900);
}
...
function playPlaybackSfx(event) {
  ...
  if (event.event === "SHUFFLE" || event.event === "SUB_PC_BOTTOM") {
    ...
    window.setTimeout(playAttackSfx, 340);
...
function renderDeckSelect() {
  ...
  ui.deckOptions.replaceChildren(...decks.map((deck, index) => {
    ...
    button.innerHTML = `<span class="deck-box-sprite"><img src="/assets/ui/${deck.id}-deck-box.png" alt="${escapeHtml(deck.name)} deck box"></span><span class="deck-choice-copy"><strong>${escapeHtml(deck.name)}</strong><span>50 MAIN · 20 CHEER · 2 OSHI OPTIONS</span></span>`;
...
function transferDrawCards(cardIds) {
  return cardIds.map((cardId, index) => {
    const definition = card(cardId);
    return `<img class="mini-card draw-card transfer-draw-card" src="${imagePath(definition)}" alt="${escapeHtml(definition?.name ?? "Cheer")}" style="--draw-index:${index}">`;
  }).join("");
}
...
function renderPlaybackVisual(event, frame) {
  ...
  if (event.event === "ART") {
    ...
    ui.playbackVisual.classList.add("attack-window");
    ui.playbackVisual.innerHTML = `<div class="attack-visual cinematic-attack attack-${safeColor}"><span class="cinematic-card attacker-card">${playbackCard(data.attackerCardId)}</span><span class="playback-arrow">➜</span><span class="cinematic-card target-card">${playbackCard(data.targetCardId)}<span class="attack-effect">${particles}</span><span class="hit-burst">✦</span></span></div>`;
  ...
  if (event.event === "ARCHIVE_CHEERS_ATTACHED") {
    ...
    const cheers = cheerIds.length ? transferDrawCards(cheerIds) : `<span class="playback-caption">NO CHEER SELECTED</span>`;
    ui.playbackVisual.innerHTML = `<div class="cheer-transfer">${playbackEndpoint(data.sourceCardId, card(data.sourceCardId)?.name)}<div class="draw-sequence transfer-stream">${cheers}</div>${playbackEndpoint(data.targetCardId, card(data.targetCardId)?.name)}</div>`;
    return;
  }
  if (event.event === "CHEER_REATTACHED") {
    ui.playbackVisual.innerHTML = `<div class="cheer-transfer">${playbackEndpoint(data.sourceCardId, data.sourceName)}<div class="draw-sequence transfer-stream">${transferDrawCards([data.cardId])}</div>${playbackEndpoint(data.targetCardId, data.targetName)}</div>`;
    return;
  }
...
function playbackAnimationDuration(event) {
  ...
  if (event.event === "ARCHIVE_CHEERS_ATTACHED") return 620 + Math.max(0, Number(event.data?.count ?? event.data?.cheerCardIds?.length ?? 1) - 1) * 260;
  if (event.event === "CHEER_REATTACHED") return 620;
  if (event.event === "ART" || event.event === "SHUFFLE") return 850;
  return 320;
}

...
test("browser client guides setup, hides CPU secrets, and completes a match", async t => {
  ...
  assert.match(html, /Attack_Damage\.wav/);
  ...
  assert.match(script, /DECISION_SELECT_STAGE_CHEER/);
  assert.match(script, /DECISION_SELECT_AMAZING_PC_HOLOMEM/);
  ...
  assert.match(script, /activateBattleCard\(owner, owner, holomem\.stageId\)/);
  ...
  assert.match(script, /handDisplayOrder/);
  assert.match(script, /newcomers\.length\) handDisplaySorted = false/);
  assert.doesNotMatch(script, /handSortEnabled/);
  assert.match(script, /SELECT_REATTACH_CHEER/);
  assert.match(script, /SELECT_OPPONENT_BACK/);
  assert.match(script, /SELECT_HOLO_POWER_CARD/);
  assert.match(script, /SELECT_FIRST_GRAVITY_CARDS/);
  assert.match(script, /CHOOSE_OPTIONAL_ROLL/);
  assert.match(script, /DECLARE_DIE_FACE/);
  assert.match(script, /oshi-action-summary/);
  assert.match(script, /art-action-summary/);
  assert.match(script, /function fitArtActionHeadings\(\)/);
  assert.match(script, /function fitDialogTitle\(title\)/);
  assert.match(script, /openBattleCardActions\(item\.owner, item\.key\)/);
  assert.match(script, /TABLE_TARGET_COPY/);
  assert.match(script, /function beginTableTarget\(mode\)/);
  assert.match(script, /function focusableBattleCards\(row\)/);
  assert.match(script, /cinematic-attack/);
  assert.match(script, /target-card/);
  assert.match(script, /function transferDrawCards\(cardIds\)/);
  assert.match(script, /assets\/ui\/\$\{deck\.id\}-deck-box\.png/);
  assert.match(script, /SFX_SYNTH_BOOST = 3/);
  assert.match(script, /ui\.attackSfx\.play\(\)/);
  assert.match(script, /CHEER_REATTACHED/);
  assert.match(script, /sourceName/);
  assert.match(script, /CHEER DECK/);
  assert.match(script, /flow-skill-confirm/);
  assert.match(script, /dialogStack = \[\];[\s\S]{0,80}showScreen\("DIALOG"\);[\s\S]{0,80}openCardActions\(item\)/);
  ...
  assert.match(script, /function beginTitleMenu\(\)/);
  assert.match(script, /function restartTitleIntro\(\)/);
  assert.match(script, /restartTitleIntro\(\);[\s\S]{0,80}showScreen\("BOOT"\)/);
  assert.equal((script.match(/restartTitleIntro\(\)/g) ?? []).length, 2, "the intro restart is declared once and called only after forfeit");
  ...
  assert.match(styles, /title-white-fade/);
  assert.match(styles, /title-background-settle/);
  assert.match(styles, /press-start-flash/);
  assert.match(styles, /console-title-screen \{[\s\S]{0,180}Endless Pixel/);
  assert.match(styles, /console-title-screen \{[\s\S]{0,220}font-smooth: never/);
  assert.match(styles, /press-start-prompt::before[\s\S]{0,150}command-cursor/);
  assert.match(styles, /press-start-prompt \{[\s\S]{0,400}background: transparent[\s\S]{0,120}border: 0/);
  ...
  assert.match(styles, /official-title-logo[\s\S]{0,160}image-rendering: pixelated/);
  assert.doesNotMatch(styles, /\.official-title-logo \{[^}]*filter:/);
  assert.match(styles, /console-title-logo h1[\s\S]{0,220}font-size: 20px/);
  assert.match(styles, /@font-face[\s\S]{0,180}EndlessPixel\.ttf/);
  assert.match(styles, /attack-window/);
  assert.match(styles, /cinematic-card/);
  assert.match(styles, /damage-flicker/);
  assert.doesNotMatch(styles, /battlefield-playback-stage/);
  assert.match(styles, /card-token\.targetable/);
  assert.match(styles, /console-menu-button[\s\S]{0,420}-webkit-text-stroke/);
  assert.match(styles, /#oshi-select, \.flow-screen[\s\S]{0,180}title-splash\.png/);
  assert.match(styles, /#oshi-select \.large-dialog, \.flow-screen \.flow-dialog[\s\S]{0,120}rgb\(248 248 244 \/ 82%\)/);
  assert.match(styles, /flow-card-list[\s\S]{0,140}rgb\(231 236 239 \/ 78%\)/);
  assert.match(styles, /art-action-entry/);
  assert.match(styles, /oshi-action-entry/);
  assert.match(styles, /oshi-action-effect/);
  assert.match(styles, /art-action-heading strong[\s\S]{0,120}flex: 1 1 auto/);
  assert.doesNotMatch(styles, /art-action-heading strong \{[^}]*text-overflow: ellipsis/);
  assert.match(styles, /dialog-title-row h2[\s\S]{0,120}white-space: nowrap/);
  ...
  const logoResponse = await fetch(`${base}/assets/ui/Hololive_Logo.png`);
  assert.equal(logoResponse.status, 200);
  assert.equal(logoResponse.headers.get("content-type"), "image/png");
  const logoHash = createHash("sha256").update(Buffer.from(await logoResponse.arrayBuffer())).digest("hex");
  assert.equal(logoHash, "7071edaf4101028979c88d9114d92b167e18b42b8a9d4b750411f577cc73b405");
  ...
  const titleFontResponse = await fetch(`${base}/assets/fonts/EndlessPixel.ttf`);
  assert.equal(titleFontResponse.status, 200);
  assert.equal(titleFontResponse.headers.get("content-type"), "font/ttf");
  const deckBoxResponse = await fetch(`${base}/assets/ui/hSD01-deck-box.png`);
  assert.equal(deckBoxResponse.status, 200);
  assert.equal(deckBoxResponse.headers.get("content-type"), "image/png");
  const attackSoundResponse = await fetch(`${base}/assets/audio/Attack_Damage.wav`);
  assert.equal(attackSoundResponse.status, 200);
  assert.equal(attackSoundResponse.headers.get("content-type"), "audio/wav");
  ...
  let aiTossSnapshot = await post(base, "/api/new-game", { deckId: "hSD01", playerOshiId: "hSD01-001", seed: 1 });
  aiTossSnapshot = await post(base, "/api/coin-flip", { call: "TAILS" });
  assert.equal(aiTossSnapshot.flow.step, "AI_ORDER");
  assert.equal(aiTossSnapshot.flow.aiOrderChoice, "FIRST");
  assert.equal(aiTossSnapshot.flow.startingPlayer, "P2");
  ...
  redrawSnapshot = await post(base, "/api/new-game", { deckId: "hSD01", playerOshiId: "hSD01-001", seed: 92 });
  ...
  while (snapshot.state.status === "ONGOING" && requests < 500) {
    ...
    } else if (["SELECT_STAGE_CHEER", "SELECT_AMAZING_PC_HOLOMEM"].includes(snapshot.state.pendingDecision?.step)) {
      snapshot = await post(base, "/api/decision", { selectedUid: snapshot.state.pendingDecision.eligibleUids[0] });
    ...
    } else if (["SELECT_LIFE_CHEER_TARGET", "SELECT_NEW_CENTER", "SELECT_EFFECT_CHEER_TARGET", "SELECT_ARCHIVE_CHEER_TARGET", "SELECT_REATTACH_TARGET", "SELECT_OPPONENT_BACK", "SELECT_ARCHIVE_CHEER_DESTINATION"].includes(snapshot.state.pendingDecision?.step)) {
    ...
    } else if (["SELECT_REATTACH_CHEER", "SELECT_HOLO_POWER_CARD", "SELECT_HAND_FOR_HOLO_POWER", "SELECT_ARCHIVE_CHEER"].includes(snapshot.state.pendingDecision?.step)) {
      snapshot = await post(base, "/api/decision", { selectedUid: snapshot.state.pendingDecision.eligibleUids[0] });
    } else if (snapshot.state.pendingDecision?.step === "SELECT_OPTIONAL_ARCHIVE_CHEER") {
      snapshot = await post(base, "/api/decision", { selectedUid: null });
    } else if (snapshot.state.pendingDecision?.step === "SELECT_FIRST_GRAVITY_CARDS") {
      snapshot = await post(base, "/api/decision", { selectedUids: [] });
    } else if (snapshot.state.pendingDecision?.step === "ORDER_FIRST_GRAVITY_BOTTOM") {
      snapshot = await post(base, "/api/decision", { orderedUids: snapshot.state.players.P1.resolution.map((x: {uid:string}) => x.uid) });
    } else if (snapshot.state.pendingDecision?.step === "CHOOSE_OPTIONAL_ROLL") {
      snapshot = await post(base, "/api/decision", { choice: false });
    } else if (snapshot.state.pendingDecision?.step === "CHOOSE_DIE_METHOD") {
      snapshot = await post(base, "/api/decision", { choice: false });
    } else if (snapshot.state.pendingDecision?.step === "DECLARE_DIE_FACE") {
      snapshot = await post(base, "/api/decision", { number: 1 });
    } else if (snapshot.state.pendingDecision?.step === "CHOOSE_EXPANDING_MAP_RETURN") {
      snapshot = await post(base, "/api/decision", { choice: false });
    } else {

...
const summary = {
  ...
  averageKnockouts: Number((results.reduce((sum, x) => sum + x.state.log.filter(e => e.event === "DOWNED").length, 0) / Math.max(1, games)).toFixed(2)),
  endings: {
    life: results.filter(x => Object.values(x.state.lossReasons).flat().some(reason => reason.includes("Life"))).length,
    deckOut: results.filter(x => Object.values(x.state.lossReasons).flat().some(reason => reason.includes("draw during"))).length,
    emptyStage: results.filter(x => Object.values(x.state.lossReasons).flat().some(reason => reason.includes("Stage"))).length,
    redraw: results.filter(x => Object.values(x.state.lossReasons).flat().some(reason => reason.includes("redraw count"))).length,
  },
};

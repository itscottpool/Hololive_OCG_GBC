...
function recordStep(playback: PlaybackFrame[], actionType: string, playerId: PlayerId | undefined, operation: () => void): void {
  ...
  if (events.length) playback.push({ actionType, playerId, events, state: projectState() });
}
...
function startPregame(deckId: string, playerOshiId: string, requestedSeed?: number): void {
  ...
  const seed = Number.isSafeInteger(requestedSeed) ? requestedSeed! : Date.now() % 2_147_483_647;
  engine = null;
  session = {
    deckId,
    playerOshiId,
...
function flipCoin(call: "HEADS" | "TAILS"): void {
  ...
  if (session.coinWinner === "P1") {
    ...
    // This aggressive deck prefers to act second: it may use LIMITED support
    // cards and becomes the first player allowed to perform Arts.
    session.aiOrderChoice = "SECOND";
    session.startingPlayer = "P1";
...
function contentType(filePath: string): string {
  ...
  return ({
    ...
    ".png": "image/png", ".jpg": "image/jpeg", ".mp3": "audio/mpeg", ".json": "application/json; charset=utf-8",

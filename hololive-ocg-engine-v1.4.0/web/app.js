...
const ids = [
  "boot", "boot-menu", "story-button", "vs-ai-button", "options-button", "exit-button",
  ...
  "dialog-zone-toggle", "card-detail", "dialog-help", "action-playback", "playback-visual", "playback-speaker", "playback-text", "playback-help",
  "inspect-dialog", "inspect-image", "inspect-name", "battle-music", "music-toggle",
...
let bgmLevel = readSavedLevel("hololive-ocg-bgm-level", 3);
let sfxLevel = readSavedLevel("hololive-ocg-sfx-level", 10);

ui.battleMusic.volume = bgmLevel / 10;
...
function updateMusicButton() {
  ui.musicToggle.textContent = musicEnabled && !ui.battleMusic.paused ? "♫" : "♪×";
  ui.musicToggle.title = musicEnabled ? "Mute music" : "Play music";
}

function startMusic() {
  if (!musicEnabled || bgmLevel === 0) return;
  ui.battleMusic.play().then(updateMusicButton).catch(() => updateMusicButton());
}

function toggleMusic() {
  musicEnabled = !musicEnabled;
  if (musicEnabled) startMusic();
  else ui.battleMusic.pause();
  updateMusicButton();
}
...
function playPlaybackSfx(event) {
  ...
  } else if (event.event === "ARCHIVE_CHEERS_ATTACHED" || event.event === "EFFECT_SEND_CHEER" || event.event === "CHEER_ATTACHED") {
...
function renderFlowCardList(items, multi = false, ordered = false, activateOnClick = false) {
  ...
  items.forEach((item, index) => {
    ...
    row.className = `flow-card-row${index === flowIndex ? " selected" : ""}${chosen ? " chosen" : ""}${item.disabled ? " disabled" : ""}`;
...
const PLAYBACK_EVENTS = new Set([
  ...
  "ARCHIVE_CHEERS_ATTACHED", "COLLAB_RETURNED",
...
function renderPlaybackVisual(event, frame) {
  ...
  if (event.event === "ARCHIVE_CHEERS_ATTACHED") {
    ...
    ui.playbackVisual.innerHTML = `<div class="cheer-transfer">${playbackCard(data.sourceCardId)}<div class="transfer-stream">${cheers}</div>${playbackCard(data.targetCardId)}</div>`;
    return;
  }
...
function playbackAnimationDuration(event) {
  ...
  if (event.event === "ART" || event.event === "SHUFFLE" || event.event === "ARCHIVE_CHEERS_ATTACHED") return 850;
  return 320;
}
...
async function playResponse(nextModel, previousModel) {
  ...
  for (const frame of frames) {
    model = { ...nextModel, state: frame.state, legalActions: [], playback: [] };
    showScreen("PLAYBACK");
    renderBattle();
    for (const event of frame.events.filter(entry => PLAYBACK_EVENTS.has(entry.event))) {
      await showPlaybackEvent(event, frame);
    }
  }
...
function renderFlow() {
  ...
  if (step === "CENTER") {
    ...
    const selected = candidates[flowIndex];
    setFlowActions([flowButton("SET AS CENTER", () => selected && flowPost("/api/setup/center", { cardUid: selected.uid }), true)]);
    return;
  }
  ...
  if (step === "DECISION_SELECT_DEBUT") {
    ...
    const selected = items[flowIndex];
    setFlowActions([flowButton("PLACE ON BACK", () => selected && flowPost("/api/decision", { selectedUid: selected.uid }), true)]);
  ...
  if (step === "DECISION_SELECT_LIMITED") {
    ...
    const selected = items[flowIndex];
    const actions = [];
    if (selected && eligible.has(selected.uid)) actions.push(flowButton("ADD TO HAND", () => flowPost("/api/decision", { selectedUid: selected.uid }), true));
    actions.push(flowButton("TAKE NO CARD", () => flowPost("/api/decision", { selectedUid: null }), !(selected && eligible.has(selected.uid))));
    setFlowActions(actions);
    ui.flowHelp.textContent = "↑ ↓ REVIEW · ENTER CONFIRM";
    return;
  }
  ...
  if (step === "DECISION_SELECT_LIFE_CHEER_TARGET") {
    ...
    renderFlowCardList(items, false, false, true);
    const selected = items[flowIndex];
    setFlowActions([flowButton("ATTACH CHEER", () => selected && flowPost("/api/decision", { selectedStageId: selected.uid }), true)]);
  ...
  if (step === "DECISION_SELECT_NEW_CENTER") {
    ...
    renderFlowCardList(items, false, false, true);
    const selected = items[flowIndex];
    setFlowActions([flowButton("MOVE TO CENTER", () => selected && flowPost("/api/decision", { selectedStageId: selected.uid }), true)]);
  ...
  if (step === "DECISION_SELECT_EFFECT_CHEER_TARGET") {
    ...
    renderFlowCardList(items, false, false, true);
    const selected = items[flowIndex];
    setFlowActions([flowButton("SEND CHEER", () => selected && flowPost("/api/decision", { selectedStageId: selected.uid }), true)]);
  ...
  if (step === "DECISION_SELECT_ARCHIVE_CHEER_TARGET") {
    ...
    renderFlowCardList(items, false, false, true);
    const selected = items[flowIndex];
    setFlowActions([flowButton("CHOOSE HOLOMEM", () => selected && flowPost("/api/decision", { selectedStageId: selected.uid }), true)]);
...
function summary(player, opponent = false) {
  ...
  return `<div class="summary-oshi">${opponent ? "CPU STATUS" : "PLAYER STATUS"}</div>
    <div class="summary-row"><span>HAND</span><span>${player.handCount}</span></div><div class="summary-row"><span>DECK</span><span>${player.deckCount}</span></div>
    <div class="summary-row"><span>ARCHIVE</span><span>${player.archiveCount}</span></div><div class="summary-row"><span>STAGE</span><span>${stageCount}/6</span></div>`;
}
...
function stageToken(holomem, position, owner) {
  ...
  const related = (model.legalActions ?? []).some(action => action.targetStageId === holomem.stageId || action.attackerStageId === holomem.stageId);
  ...
  token.addEventListener("click", () => openVisibleStage(owner, holomem.stageId));
...
function waitingCheerCardId(player, owner) {
  ...
  return ["SELECT_LIFE_CHEER_TARGET", "SELECT_EFFECT_CHEER_TARGET"].includes(decision.step) ? decision.cardId ?? null : null;
}
...
function oshiToken(player, owner) {
  ...
  token.addEventListener("click", () => openZone("OSHI", null, owner));
...
function renderBackstagePeeks(player, container, owner) {
  ...
  container.replaceChildren(...player.stage.back.filter(Boolean).map(holomem => {
    ...
    peek.addEventListener("click", () => openZone("BACKSTAGE", holomem.stageId, owner));
    return peek;
  }));
}
...
function renderCommands() {
  ...
  ui.commandMenu.replaceChildren(...COMMANDS.map((name, index) => {
    ...
    button.className = `command${battleFocus === "COMMAND" && index === commandIndex ? " selected" : ""}${enabled[name] ? "" : " disabled"}${recommended === name ? " recommended" : ""}`;
    button.textContent = name === "STEP" ? nextStepLabel() : name;
    button.addEventListener("mouseenter", () => { battleFocus = "COMMAND"; commandIndex = index; renderBattleFocus(); });
    button.addEventListener("click", () => { battleFocus = "COMMAND"; commandIndex = index; selectCommand(name); });
    return button;
  }));
}
...
function renderBattleFocus() {
  document.querySelectorAll(".card-token.focused, .backstage-peek-card.focused").forEach(token => token.classList.remove("focused"));
  ...
  const items = visibleBattleCards(battleFocus);
  ...
  ui.eventMessage.textContent = `${item.owner === "P1" ? "YOUR" : "OPPONENT"} ${area} · ${definition?.name ?? "CARD"} · ← → SELECT · ENTER OPEN`;
}

function focusVisibleCard(row, key) {
  battleFocus = row;
  const index = visibleBattleCards(row).findIndex(item => item.key === key);
  stageFocusIndex[row] = Math.max(0, index);
  renderBattleFocus();
}

function moveBattleRow(delta) {
  const rows = BATTLE_ROWS.filter(row => row === "COMMAND" || visibleBattleCards(row).length);
...
function activateVisibleFocus() {
  const item = visibleBattleCards(battleFocus)[stageFocusIndex[battleFocus]];
  if (!item) return;
  if (item.kind === "OSHI") openZone("OSHI", null, item.owner);
  else if (item.zone.startsWith("BACK")) openZone("BACKSTAGE", item.key, item.owner);
  else openVisibleStage(item.owner, item.key);
}
...
function relatedActions(item) {
  ...
  if (item.kind === "FIELD" && item.owner === "P1") return actions.filter(action => {
    ...
    return action.targetStageId === item.holomem.stageId;
  });
  return [];
}
...
function openVisibleStage(owner, selectedStageId) {
  const items = visibleStageItems(owner);
  const selectedIndex = items.findIndex(item => item.holomem.stageId === selectedStageId);
  dialogStack = [{ title: owner === "P1" ? "YOUR STAGE" : "OPPONENT STAGE", items, index: Math.max(0, selectedIndex), level: "CARDS" }];
  showScreen("DIALOG");
  renderDialog();
}
...
function openZone(command, selectedStageId = null, fieldOwner = "P1") {
  let items = [];
  if (command === "HAND") items = model.state.players.P1.hand.map(instance => {
    const definition = card(instance.cardId);
    return { kind: "HAND", uid: instance.uid, cardId: instance.cardId, name: definition.name, tag: "", cheerColors: definition.type === "Holomem" ? [] : undefined };
  });
...
function openCardActions(item) {
  ...
  const items = [
    ...
    ...[...groups.values()].map(group => ({ kind: "ACTION", name: actionName(group[0], item), tag: "ACTION", group, source: item })),
...
function chooseActionGroup(item) {
  ...
  if (alwaysChooseTarget || item.group.length > 1) {
    const targets = item.group.map(candidate => {
      const target = allStageCards().find(stage => stage.holomem.stageId === candidate.targetStageId);
      return { kind: "TARGET", name: targetName(candidate), tag: "TARGET", cardId: target?.cardId, holomem: target?.holomem, zone: target?.zone, owner: target?.owner, action: candidate };
    });
    dialogStack.push({ title: `${item.name} — TARGET`, items: targets, index: 0, level: "TARGETS", source: item.source });
    renderDialog();
...
function renderDialog() {
  ...
  ui.dialogTitle.textContent = sideToggleRoot ? `${view.title} — ${sideToggleRoot.fieldOwner === "P1" ? "YOURS" : "OPPONENT"}` : view.title;
  ...
  ui.dialogList.replaceChildren(...view.items.map((item, index) => {
    ...
    row.className = `list-entry${index === view.index ? " selected" : ""}${item.kind === "ATTACHMENT" ? " attachment-entry" : ""}${firstAttachment ? " attachment-first" : ""}`;
    ...
    row.innerHTML = `${attachmentHeading}<span class="cursor">${index === view.index ? "▶" : ""}</span><span class="list-entry-name"><span>${escapeHtml(item.name)}</span>${cheerIcons}</span><span class="tag">${escapeHtml(item.tag ?? "")}</span>`;
  ...
  ui.dialogHelp.textContent = sideToggleRoot ? `← → YOUR/OPPONENT ${view.title} · ↑ ↓ CARD · ENTER ACTIONS` : view.level === "CARDS" ? "↑ ↓ CARD · ENTER VIEW ACTIONS · ESC CLOSE" : view.level === "ACTIONS" ? "↑ ↓ ACTION · ENTER SELECT · ESC CARDS" : "↑ ↓ TARGET · ENTER CONFIRM · ESC ACTIONS";
}
...
async function submitAction(action) {
  ...
  try {
    ...
    dialogStack = [];
    await playResponse(nextModel, previousModel);
  } catch (error) { showToast(error.message); renderDialog(); }
  finally { busy = false; }
}
...
function handleModel() {
  ...
  if (model?.state?.pendingDecision || (model?.flow && model.flow.step !== "PLAYING")) return renderFlow();
  showScreen("BATTLE");
  renderBattle();
}
...
document.addEventListener("keydown", event => {
  ...
  if (screen === "BOOT") {
    ...
    if (event.key === "ArrowUp") moveFlow(-1);
      ...
      if (["BACK", "BOTTOM", "DECISION_ORDER_BOTTOM", "DECISION_SELECT_ARCHIVE_CHEERS"].includes(currentStep)) {
        toggleFlowCard(flowItems[flowIndex].uid, currentStep === "DECISION_ORDER_BOTTOM");
      }
    ...
    else if (event.key === "Escape") { promptForfeit(); return; }
    renderCommands();
    renderBattleFocus();

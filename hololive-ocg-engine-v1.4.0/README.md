...
This project is a dependency-free TypeScript implementation of the Hololive Official Card Game rules needed to play the supplied hSD01 Tokino Sora/AZKi starter deck. Version 1.4.0 introduces physical card-pile rendering and a complete attached-card browser while retaining the comprehensive-rulebook-accurate setup and console-style interface.
...
The opening screen places the game logo and a console-style menu directly over the pixel-treated title artwork. Story Mode is visibly reserved but disabled; Vs. AI now opens a deck-selection screen before Oshi selection, Options contains Color Scheme and separate BGM/SFX volume controls, and Exit Game stops the local process. The three interface palettes—Classic White, Retro Mint, and Moonlight—and both audio levels are remembered for later sessions on the same browser.

Every performed action now plays before control returns to the player. Text windows narrate CPU and player decisions in chronological order, with card reveals, Cheer displays, draw-to-hand movement, deck shuffling, phase announcements, and two-card attack/hit scenes. After each animation and message finish, press Enter or click to advance to the next action. Nothing advances automatically.
...
- `assets/ui`: card back, Cheer symbols, and the 480×270 pixel-treated title splash.
- `assets/audio`: the looping `Stellar_Stellar_Chiptune.mp3` demo track.
